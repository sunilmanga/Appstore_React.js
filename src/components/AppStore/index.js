...
export default class AppStore extends Component {
  state = {ca: tabsList[0].tabId}

  chng = tabId => {
    this.setState({ca: tabId})
  }

  render() {
    const {ca} = this.state
    const ne = appsList.filter(eee => ca === eee.category)
    console.log(ne)
    return (
      <div>
        <h1>App Store</h1>
        ...
        <div>
          <ul>
            {tabsList.map(e => (
              <TabItem e={e} key={e.tabId} chng={this.chng} />
            ))}
          </ul>
        </div>
        <div>
